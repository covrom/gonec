...
<body bgcolor=#25282c>
	...
	<div id="wrap">
		<div id="code" name="code">Функция бенч1()
	дтнач = ТекущаяДата()
	...
	серв = Новый Сервер
	Попытка
		серв.Открыть("tcp", "127.0.0.1:9990", 100, ОбработатьСерв, 0)
	Исключение
...
Функция бенч2()
	...
	серв = Новый Сервер
	Попытка
		серв.Открыть("http", "127.0.0.1:9990", 100, {"/test":ОбработатьHTTP}, 0)
	Исключение
		Сообщить(ОписаниеОшибки())
		Сообщить("Кажется сервер уже запущен, или тут какая-то другая ошибка, но мы все равно попробуем отправить запрос :)")
	КонецПопытки
...
Сообщить("Все завершилось просто идеально: TCP за", бенч1(), ", HTTP за", бенч2(), "!")
