...
<body bgcolor=#25282c>
	...
	ОбработатьСерв = Функция (соед)
		...
		база.НачатьТранзакцию(Истина)
		Попытка
			база.Таблица("testing").Установить(Формат("%s",соед), запр)
			база.ЗафиксироватьТранзакцию()
		Исключение
			база.ОтменитьТранзакцию()
	...
	серв = Новый Сервер
	Попытка
		серв.Открыть("tcp", "127.0.0.1:9990", 100, ОбработатьСерв, бд)
	Исключение
...
Функция бенч2(бд)
	...
	ОбработатьHTTP = Функция (вых,вх)
		...
		база.НачатьТранзакцию(Истина)
		Попытка
			база.Таблица("testing").Установить(вх.Адрес(), запр)
			база.ЗафиксироватьТранзакцию()
	...
	серв = Новый Сервер
	Попытка
		серв.Открыть("http", "127.0.0.1:9990", 100, {"/test":ОбработатьHTTP}, бд)
	Исключение
		Сообщить(ОписаниеОшибки())
		Сообщить("Кажется сервер уже запущен, или тут какая-то другая ошибка, но мы все равно попробуем отправить запрос :)")
	КонецПопытки
...
база.НачатьТранзакцию(Ложь)
Попытка
	Сообщить(база.Таблица("testing").ПолучитьВсе())
	база.ЗафиксироватьТранзакцию()
Исключение
	база.ОтменитьТранзакцию()
КонецПопытки
